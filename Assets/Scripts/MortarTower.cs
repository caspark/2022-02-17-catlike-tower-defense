...
public class MortarTower : Tower {
    ...
    void OnValidate() {
        float enemyColliderRadius = 0.125f;
        float x = targetingRange + enemyColliderRadius * 2f + 0.00001f;
    ...
    public override void GameUpdate() {
        ...
        while (launchProgress >= 1f) {
            if (AcquireTarget(out TargetPoint target)) {
                ...
                // nothing to shoot at, so mark as ready to fire next update
    ...
    public void Launch(TargetPoint target) {
        ...
        if (visualizeTrajectory) {
            ...
            for (int i = 1; i <= 10; i++) {
                ...
                Debug.DrawLine(prev, next, Color.blue, debugDrawDuration);
                prev = next;
            }

            Debug.DrawLine(launchPoint, targetPoint, Color.yellow, debugDrawDuration);
            Debug.DrawLine(
